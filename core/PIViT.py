...
class SwinTransformerBlock3D(nn.Module):
    ...
    def __init__(
        ...
        self.attn = WindowAttention3D(
            dim, window_size=self.window_size, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop
        )
...
class PIViT(LoadableModel):
    @store_config_args
    def __init__(self, in_shape=(80, 96, 80), in_channel=1, start_channel=32):
        ...
        for i in range(4):
            self.transformer.append(SpatialTransformer([s // 2**i for s in in_shape]))
        self.up = nn.Upsample(scale_factor=2, mode="trilinear", align_corners=True)

    def forward(self, fix, mov):
        fx1, fx2, fx3 = self.encoder(mov)
        fy1, fy2, fy3 = self.encoder(fix)
...
if __name__ == "__main__":
    model = PIViT().cuda()
    fix = torch.randn(1, 1, 80, 96, 80).cuda()
    mov = torch.randn(1, 1, 80, 96, 80).cuda()
    flow = model(fix, mov)
