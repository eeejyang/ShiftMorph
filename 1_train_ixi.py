...
parser.add_argument("--lmd_smooth", type=float, default=4.0, help="weight for smoothness loss")
parser.add_argument("--lmd_dice", type=float, default=0.0, help="weight for dice loss")
...
label_shape = [1, 30, *spatial_size]
save_index_keys = ["similar", "dice", "loss"]

log_path = os.path.join(f"./logs/IXI-smooth={args.lmd_smooth}-dice={args.lmd_dice:.1f}", time.strftime("%Y-%m%d-%H%M", time.localtime()))
...
# 在这里面填上你的配准模型, 并实现这样的函数： flow =  model.forward(fix,  mov)
# 输入fix, mov两个图，输出形变场
# --------------------------------------------------------------------------------------
...
train_data = IXIDataset().select("train")
val_data = IXIDataset().select("val")
...
def train_one_epoch(epoch):
    ...
    for step, batch_data in enumerate(train_loader):
        ...
        if args.lmd_dice > 0:
            ...
            batch_data["moving_label"].cuda().float()
...
def evaluate(epoch):
    ...
    for step, batch_data in enumerate(val_loader):
        ...
        mov_img = batch_data["moving_image"].cuda().float()
        mov_seg = batch_data["moving_label"].cuda().float()
